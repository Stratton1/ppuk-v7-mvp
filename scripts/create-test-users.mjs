 ...
 * Usage: cd frontend && node ../scripts/create-test-users.mjs
 * 
 * Requires:
...
async function createTestUsers() {
  ...
  for (const user of TEST_USERS) {
    try {
      ...
      console.error(`❌ Error processing ${user.email}:`, error instanceof Error ? error.message : String(error));
    }
  }
...
async function main() {
  try {
    ...
    TEST_USERS.forEach(user => {
      console.log(`   ${user.email} (${user.primary_role}) - Password: ${user.password}`);
    });
  } catch (error) {
    console.error('\n❌ Fatal error:', error);
    process.exit(1);
